...
In Conakry, Guinea, the number of Ebola virus disease (EVD) with illness onset increased during September-October, 2014. Conakry is approximately 450 miles from the forrest region (Gueckedou, Macenta, Nzérékoré, and Beyla districts), where most EVD cases have occured.  During the same period, cases also increased in Kindia, the district adjacent to Conakry.  There were `r data %>% count()` new cases of EVD in Guinea.  Of these, `r data %>% filter(Region %in% 'Conakry') %>% count()` were identified in Conakry, `r data %>% filter(Region %in% 'Kindia') %>% count()` in Kindia, and `r data %>% filter(Region %in% 'Nzérékoré') %>% count()` in the forrest region.  The median age of new EVD cases in Guinea was 
`r round(median(data$age, na.rm=T)/12,0)` years [range: `r min(data$age, na.rm=T)` months - `r round(max(data$age, na.rm=T)/12, 0)` years];
`r round(median(data.cky$age, na.rm=T)/12,0)` years [`r min(data.cky$age, na.rm=T)` months - `r round(max(data.cky$age, na.rm=T)/12,0)` years] in Conakry; 
`r round(median(data.kindia$age, na.rm=T)/12,0)` years [`r min(data.kindia$age, na.rm=T)` months - `r round(max(data.kindia$age, na.rm=T)/12, 0)` years] in Kindia;  and, 
`r round(median(data.forrest$age, na.rm=T)/12,0)` years [`r min(data.forrest$age, na.rm=T)` months - `r round(max(data.forrest$age, na.rm=T)/12, 0)` years] in the forrest region.  

```{r case_counts, include=FALSE}
...
kable( comma(y), align = c('l', rep('r', ncol(y)-1)) )
```


```{r epicurve_facet_sc, fig.height=8, fig.width=6.5, fig.cap='Weekly Case Count by Region, Guinee'}

#kindia geography

# data.kindia %>%  count(DistrictRes, ParishRes, SCRes, VillageRes)
  
cky.kindia = rbind(data.cky, data.kindia)

# facet by region; fill by prefecture
SClevels = unique(toupper(cky.kindia[order(cky.kindia$Region, cky.kindia$SCRes), 'SCRes']))

 cky.kindia = cky.kindia %>%
  mutate( 
...
# SCRes = 'RATOMA'
# cky.kindia[ agrepl(SCRes, cky.kindia$SubCounty, max =1), 'SubCounty' ] = SCRes
# SCRes = 'MATOTO'
# cky.kindia[ agrepl(SCRes, cky.kindia$SubCounty, max =1), 'SubCounty' ] = SCRes
# SCRes = 'MATAM'
# cky.kindia[ agrepl(SCRes, cky.kindia$SubCounty, max =1), 'SubCounty' ] = SCRes
...
# cky.kindia[ cky.kindia$SubCounty %in% exceptions, 'SCRes' ] = NA
...
# It is critical to monitor the number of new cases who were known contacts.
...
When Ebola symptoms are identified in a person, a public health alert is triggered to provide effective patient care, stop disease transmission, and offer community education. To receive health care, the suspect patient is transported to the Ebola treatment center (ETC) and placed in isolation. Specimens are taken for laboratory confirmation, with the results usually being available within 24 hours. Affected persons are hospitalized until their laboratory specimens are negative. When an initial laboratory result is negative, the patient is tested again the next day.
 
A case notifications form, including demographic, clinical, epidemiologic, and laboratory information, is completed at the ETC for all patients, including suspect, probable and laboratory-confirmed at the ETC. Data collected on the form are entered into an Excel® (Microsoft Corp., Redmond, Washington) spreadsheet by the ETC staff and sent every evening to the World Health Organization (WHO) and Guinean Ministry of Health (MoH). Data sent from the two Guinean ETCs (i.e., Conakry and Guéckedou) are merged into the Epi Info™ (CDC, Atlanta, Georgia) viral hemorrhagic fever (VHF) national database, updated, and analyzed daily. Additionally, as laboratory results become available, they are reported by e-mail to the ETC and WHO/MoH to update the national database.
 
To stop disease transmission, information regarding contacts is also obtained for all identified Ebola patients. Contacts are systematically monitored for symptoms for 21 days (i.e., wellness checks and temperature monitoring daily); any contact demonstrating Ebola symptoms is placed in isolation, tested for the virus, and asked about their contacts. If a contact does not have any symptoms after 21 days, the contact is not at risk for Ebola. Contact tracing teams and field supervisors are responsible for the prompt and full implementation of contact tracing activities, and the contact tracing process repeats with each newly identified patient until no new cases occur. Contact tracing information is also sent to WHO/MoH to update information in the national database.
 

# Other stuff to include…

![network diagram](igraph.png)

•  Chain of transmission diagrams related to social network diagrams.
