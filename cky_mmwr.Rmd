...
d = data.cky %>% 
  ...
  left_join(sourceCases[, c('source', 'sourceDistrict', 'sourceSCRes', 'sourceRegion')], 
            by = 'source') %>%
  mutate( knownContact = ifelse(is.na(sourceRegion), 'Followed', 'Not Followed*') , 
          daysToIso = as.numeric( as.Date(DateIsolationCurrent) - as.Date(DateOnset) ),
          sourceRegion = ifelse(is.na(sourceRegion), 'Source case unidentified', 
                                ifelse(sourceRegion %in% 'Conakry', 'Conakry', 'Other')
                                ) 
          ) %>%
  select( ID, knownContact, sourceRegion, daysToIso, Age, HCW, Gender) %>% 
  group_by( knownContact, sourceRegion, ID) %>%
  summarize(  n = 1) 
, 
              Age = sprintf("%s", 
                            format(median(floor(ages/12), na.rm = TRUE), nsmall = 1) ),
...
g = ggplot( ) + theme_map +
  ...
  geom_point(data= map.data.p.c, aes(x=V1, y=V2, size = SepOct), 
             color = "black") + 
...
# ggsave(plot = g, 'SepOct.png', width=6.5, height=6.5, units='in')
```
