...
# sas import...
# export to csv; open in excel and re-export as csv!!!
# options(stringsAsFactors=FALSE)
# library(data.table)
# file.name = "case_guinea_sl_xl" 
file = paste0(dir, file.name, '.csv') 
# case_data = fread( file , sep =',')

options(stringsAsFactors=FALSE)
# library(sas7bdat)
# file.name = "case_guinea_sl.sas7bdat" 
# file = paste0(dir, file.name) 
# stringsAsFactors = FALSE
# case_data = read.sas7bdat(file, debug=FALSE)

# Maenner
x = readLines(file, skipNul=TRUE,encoding="UTF-8", n=-1L )
case_data = read.csv(textConnection(x), header = TRUE, stringsAsFactors = FALSE)
...
attr(case_data$DateOnset, "tzone") <- "UTC"
attr(case_data$DateReport, "tzone") <- "UTC"
attr(case_data$DateHospitalCurrentAdmit, "tzone") <- "UTC"
attr(case_data$DateIsolationCurrent, "tzone") <- "UTC"


# Create separate country datasets
case_data.guinea = case_data %>% filter(Country %in% 'Guinea') %>% as.data.frame()
case_data.sl = case_data %>% filter(Country %in% 'Sierra Leone') %>% as.data.frame()

save(case_data.guinea , case_data.sl, file='case_data.rda')
...
#           dateOnset = DateOnset, 
          ...
          period = ifelse( dateOnset %within% 
                             interval(last.report.date-edays(6),
                                      last.report.date
                                      ), "Last 7 days", 
                    ifelse(dateOnset %within% 
