...
# Maenner
x = readLines(file, skipNul=TRUE,encoding="UTF-16", n=-1L )
...
```{r prefets, fig.cap="Cas d'Ebola pendant le dernier 21 jours, Guinee", eval=FALSE}
...
g = ggplot( ) + theme_map +
  geom_polygon(data= map.data.p,
               aes(x=long, y=lat, group=group, 
                   fill=Cases23), 
               color="grey50", alpha=1) + 
  geom_text(data= GIN2.c, aes(label = Name, x=V1, y=V2), size=4, hjust = 1, vjust = 2) +
...
```{r isolation_region, fig.height=8}
g + facet_grid( Region ~ .)
```
...
g = ggplot(data, aes( month(DateOnset))) +
    ...
    scale_fill_manual('Status', values = c("red", "green")) 
g
