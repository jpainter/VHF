...
d_filter = d %>% filter(daysToIso >= 0 & daysToIso < 21 & 
                          StatusAsOfCurrentDate %in% c('Décédé','Vivant')) 
...
# Survival as funcion of days to isolation
...
+ For this analysis, days to isolation were restricted to 0-20 days (results: removed ~7% of reports) and did not include last month of reports (because survival data likely not updated).  

```{r isolation_survival, fig.cap='Days to Isolation by Month of Onset, Guinea'}

# remove last month because data not updated
d_filter2 = d_filter %>% filter(month(DateOnset) < 12)


d_filter2 %>% filter(StatusAsOfCurrentDate %in% 'Décédé' ) %>% 
  select(daysToIso_Died = daysToIso) %>% summary(daysToIso)

d_filter2 %>% filter(StatusAsOfCurrentDate %in% 'Vivant' ) %>% 
  select(daysToIso_Survived = daysToIso) %>% summary(daysToIso)
...
```{r isolation_survival_rate, fig.cap='Days to Isolation by Month of Onset, Guinea'}

# remove last month because data not updated
d_filter2_rate = d_filter2 %>%
  group_by(daysToIso) %>%
  summarize( survival = sum(StatusAsOfCurrentDate == 'Vivant')/ n(),
             n = n())
...
Regional difference are apparent, although these difference could be biased by difference in data reporting.  In particular, the greater than expected survival rate for patients in Conakry suggests that outcome may not be updated.

```{r isolation_survival_rate_region, fig.cap='Days to Isolation by Month of Onset, Guinea'}
...
# Time from onset to isolation
data = cases.guinea %>% 
  select( DateOnset, DateIsolationCurrent, StatusAsOfCurrentDate, StatusReport, 
          DistrictRes) %>%
  merge(Region, all.x = TRUE, by= 'DistrictRes') %>%
...
g = ggplot(data, aes( month(DateOnset))) +
    scale_x_continuous('Month', breaks = c(1:12)) +
    geom_bar(aes(fill = StatusReport), color = 'black', 
