...
cases %>% 
  ...
  ggplot( aes(x = period , y = freq, fill = Status) ) +
  geom_bar( stat = 'identity', color = 'white') +
  scale_x_date('Week beginning', labels = date_format("%b-%dd"),
...
#   left_join(Region) %>%  # replaced with merge because of possible encoding conflict
  filter( year(dateOnset) == 2014 ) %>%
  arrange(dateOnset) %>%
  mutate( interval = week(dateOnset),
          period = as.Date(floor_date(dateOnset, "week")) ) %>%  # set time period
...
# same chart using report date, DateReport
cases %>% 
  merge(Region, all.x = TRUE, by= 'DistrictRes') %>%
#   left_join(Region) %>%  # replaced with merge because of possible encoding conflict
